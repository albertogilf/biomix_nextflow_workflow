...
target_rows <- which(commands$DATA_TYPE == "Transcriptomics" & commands$ANALYSIS == "YES")
if (nzchar(label)) {
  target_rows <- target_rows[commands$LABEL[target_rows] == label]
}

if (length(target_rows) != 1) {
  stop("Expected exactly one active transcriptomics command row.", call. = FALSE)
}
...
source(file.path(workspace, "Transcriptomics", "Biomix_DGE_GENES_LIMMA.r"), local = globalenv())
